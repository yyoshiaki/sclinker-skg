# base : GSSG/code/GeneSet_toS2G/bedgraph_to_annot.py
# modified to specify cell label to the annotation.
#module load conda2
...
def make_annot_files(bed_for_annot, bimfile, annot_file, bedname):
    ...
    df_int = pd.DataFrame({'BP': bp, bedname:score})
    df_annot = pd.merge(df_bim, df_int, how='left', on='BP')
    df_annot.fillna(0, inplace=True)
    temp = df_annot[[bedname]].astype(float)
...
for numchr in range(1, 23, 1):
    ...
    make_annot_files(bed_for_annot, bimfile, annot_file, bedname)
