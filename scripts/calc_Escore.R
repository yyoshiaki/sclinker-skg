# base : https://github.com/kkdey/singlecellLDSC/blob/master/code/postprocess_ldsc.R
...
all_traits = c('UKB_460K.body_BMIz','UKB_460K.cov_EDU_YEARS','UKB_460K.lung_FVCzSMOKE','UKB_460K.cov_SMOKING_STATUS',
               ...
               'UKB_460K.disease_T2D','PASS_AgeFirstBirth','UKB_460K.disease_RESPIRATORY_ENT','UKB_460K.body_BALDING1','UKB_460K.disease_HYPOTHYROIDISM_SELF_REP', # nolint
...
run_single_enrichment_analysis = function(f annot_cell,
                                 results_cell,
                                 annotation,
                                 traits,
                                 index_in_results=1){
  enrich_table = matrix(0, 1, 3)
  
  f = "PASS_Alzheimers_Jansen2019_merged.results"
  f_log = "PASS_Alzheimers_Jansen2019_merged.log"
  res = read.table(f,header=T)
  cat("Number of annotations together with baseline : ", nrow(res), "\n")
  annot_names = as.character(res$Category)
  Mref = 5961159

  log = read.table(f_log,h=F,fill=T)
  h2g = as.numeric(as.character(log[which(log$V4=="h2:"),5]))
  myenrstat    = (h2g/Mref)*((res[,3]/res[,2])-(1-res[,3])/(1-res[,2])) #step1
  myenrstat_z  = qnorm(res[,7]/2) #step2
